 ...
 * 那么接下来则是使用，而使用的方式可以包括；接口（http/rpc）、消息监听、定时任务等方式，这些方式统一被定义为触发动作。
 *
 * @ description:抽奖服务
 * @ author: rsl
 * @ create: 2024-07-25 23:54
 **/
@Slf4j
@RestController()
        ...
        try {
            ...
            for (StrategyAwardEntity strategyAward : strategyAwardEntities) {
                ...
                raffleAwardListResponseDTOS.add(RaffleAwardListResponseDTO.builder()
                        ...
                        //(null == awardRuleLockCount) || (awardRuleLockCount <= dayPartakeCount)) ? 0 : (awardRuleLockCount - dayPartakeCount) 结果为0或awardRuleLockCount - dayPartakeCount
